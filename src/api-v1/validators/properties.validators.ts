...
export const updatePropertySchema = Joi.object({
	...
	location: Joi.object({
		addressLine: Joi.string().optional().messages({'string.base':'Address should be of type text'}),
		postalAddress: Joi.string().optional().messages({'string.base':'Address should be of type text'}),
		cityOrTown: Joi.string().optional().messages({'string.base':'City/Town should be of type text'}),
		country: Joi.string().optional().messages({'string.base':'Country should be of type text'})
	}),
...
export const getPropertySchema = Joi.object({
	...
	name:Joi.string().optional().min(3).max(100).messages({
		'string.base':'Name should be of type text',
		'string.min':'Name should ahve a minimum length of {#length} characters',
		'string.max':'Name should ahve a maximum length of {#length} characters',
	}),
	propertyType: Joi.string().optional().valid('apartment','commercial','hostel').pattern(
		new RegExp('^[a-z]+$')
	).messages({
	...
	status:Joi.string().optional().valid('available', 'not available').messages({
		'string.base':'Status should be of type text',
		'any.only':'Status can either be available or not available'
	})
})
